...
def create_enterpage():
    enter_label = tk.Label(top, text='Enter Your Nickname!', font=(None, 21), width=30, height=3,
                            borderwidth=3, relief=tk.RAISED)
    enter_label.place(x=25, y=25)

    enter_button = tk.Button(top, text='Enter', command=startgame, font=(None,20),width=10, height=1)
    enter_button.place(x=200, y=440)
...
players = []
root = tk.Tk()                # for making window
width = root.winfo_screenwidth()
height = root.winfo_screenheight()
root.geometry('{}x{}'.format(width, height))
root.title('TERNA\'S Coders')
...
button = tk.Button(ludo.get_frame(), text='ROLL', command=Dice.start,font=(None,15), width=12, height=1)
...
However, if you roll a two, you can advance the coin by two squares and then it rests there until the next move.\n 
    ...
    Enjoy the game and have fun.
        # Best of luck #
'''
tkinter.messagebox.showinfo('Get ready to play Ludo....', welcome_msg)  # MESSAGE PAGE
